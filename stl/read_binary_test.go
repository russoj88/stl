...
func Test_unitVectorFromBinary(t *testing.T) {
	b := []byte{0x4d, 0x10, 0x1c, 0x3f, 0x2a, 0xd2, 0xd3, 0xbe, 0x3a, 0x19, 0x2d, 0x3f}
	expected := UnitVector{
		Ni: 0.60962373,
		Nj: -0.4137128,
		Nk: 0.6761662,
	}
	got := unitVectorFromBinary(b)
...
func Test_coordinateFromBinary(t *testing.T) {
	b := []byte{0x4d, 0xbe, 0x6b, 0x40, 0xa6, 0xe5, 0xd2, 0x3f, 0xc1, 0x40, 0xd8, 0x40}
	expected := Coordinate{
		X: 3.68349,
		Y: 1.6476333,
		Z: 6.7579045,
	}
	got := coordinateFromBinary(b)
	if got.X != expected.X {
...
func Test_triangleFromBinary(t *testing.T) {
	b := []byte{0x8a, 0xa5, 0x10, 0x3f, 0xbd, 0x5f, 0x8f, 0x3e, 0x73, 0xae, 0x46, 0x3f, 0x4d, 0xbe, 0x6b, 0x40, 0xa6, 0xe5, 0xd2, 0x3f, 0xc1, 0x40, 0xd8, 0x40, 0xb0, 0x38, 0x5d, 0x40, 0x13, 0xc0, 0x06, 0x40, 0xc1, 0x40, 0xd8, 0x40, 0xb4, 0xad, 0x5a, 0x40, 0xbd, 0x41, 0x05, 0x40, 0xb3, 0x72, 0xd9, 0x40, 0x00, 0x00}
	expected := Triangle{
		normal: &UnitVector{
			Ni: 0.5650259,
			Nj: 0.2800273,
